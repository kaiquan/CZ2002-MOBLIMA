...
public class MovieDAO extends JSONDAO{
	...
	public void updateMovie(Movie movie){
		...
		for(int i=0;i<jsonMovies.size();i++){
			...
			if(jsonMovie.get("id").toString().equals(movie.getId())){
				...
				for(int y=0;y<movie.getCasts().size();y++){
					jsonReview.put("actor", movie.getReviews().get(y));
					jsonReviews.add(jsonCast);
				}
				jsonMovie.remove("reviews");
				jsonMovie.put("reviews", jsonCasts);
